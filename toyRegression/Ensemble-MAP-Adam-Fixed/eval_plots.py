...
for M in M_values:
    for iter in range(6):
        networks = []
        for i in range(M):
            network = ToyNet("eval_Ensemble-MAP-Adam-Fixed", project_dir="/workspace/evaluating_bdl/toyRegression").cuda()
            network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyRegression/training_logs/model_Ensemble-MAP-Adam-Fixed_%d_M%d_%d/checkpoints/model_Ensemble-MAP-Adam-Fixed_%d_M%d_epoch_150.pth" % (iter+1, M, i, iter+1, M)))
