...
for j in range(10):
    ...
    networks = []
    for i in range(M):
        network = ToyNet("eval_Ensemble-MAP-Adam-Fixed_%d_M4" % (j + 1), project_dir="/workspace/evaluating_bdl/toyRegression").cuda()
        network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyRegression/training_logs/model_Ensemble-MAP-Adam-Fixed_%d_M4_%d/checkpoints/model_Ensemble-MAP-Adam-Fixed_%d_M4_epoch_150.pth" % (j+1, i, j+1)))
