...
for M in M_values:
    for iter in range(6):
        ...
        for i in range(M):
            ...
            network = ToyNet("eval_SGLD-256", project_dir="/workspace/evaluating_bdl/toyRegression").cuda()
            network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyRegression/training_logs/model_SGLD-256_%d/checkpoints/model_SGLD-256_%d_epoch_%d.pth" % (iter+1, iter+1, int(num_epochs - i*step_size))))
