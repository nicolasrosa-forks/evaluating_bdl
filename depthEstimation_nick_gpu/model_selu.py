...
def conv_bn_selu(in_channels, out_channels, kernel_size, stride=1, padding=0, bn=True, selu=True):
    bias = not bn
    ...
    if selu:
        layers.append(nn.SELU(inplace=True))  # inplace=True means that it will modify the input directly, without allocating any additional output.
...
def convt_bn_selu(in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, bn=True, selu=True, sigmoid=False):
    bias = not bn
...
class DepthEstimationNet(nn.Module):
    def __init__(self, pretrained=False):
        ...
        self.conv1_d = conv_bn_selu(1, 32, kernel_size=3, stride=1, padding=1)
        self.conv1_img = conv_bn_selu(3, 32, kernel_size=3, stride=1, padding=1)
        
        self.conv1_1 = conv_bn_selu(32, 64, kernel_size=3, stride=1, padding=1)
        ...
        self.conv6 = conv_bn_selu(num_channels, 512, kernel_size=3, stride=2, padding=1)

        self.convt5 = convt_bn_selu(in_channels=512, out_channels=256, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.convt4 = convt_bn_selu(in_channels=(512+256), out_channels=128, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.convt3 = convt_bn_selu(in_channels=(256+128), out_channels=64, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.convt2 = convt_bn_selu(in_channels=(128+64), out_channels=64, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.convt1 = convt_bn_selu(in_channels=(64+64), out_channels=64, kernel_size=3, stride=1, padding=1)
