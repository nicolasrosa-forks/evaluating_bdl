...
def conv_bn_relu(in_channels, out_channels, kernel_size, stride=1, padding=0, bn=True, lrelu=True):
    bias = not bn
    ...
    if lrelu:
        layers.append(nn.LeakyReLU(negative_slope=0.2, inplace=True))  # inplace=True means that it will modify the input directly, without allocating any additional output.
...
def convt_bn_relu(in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, bn=True, lrelu=True):
    bias = not bn
    
    # ConvT-BN-LeakyRelu Block
    layers = []
...
class DepthEstimationNet(nn.Module):
    def __init__(self, pretrained=False):
        ...
        self.conv1_1 = conv_bn_relu(32, 64, kernel_size=3, stride=1, padding=1)
        ...
        self.convt4 = convt_bn_relu(in_channels=(512+256), out_channels=128, kernel_size=3, stride=2, padding=1, output_padding=1)
        ...
        self.convt1 = convt_bn_relu(in_channels=(64+64), out_channels=64, kernel_size=3, stride=1, padding=1)

        self.convtf_mean = conv_bn_relu(in_channels=128, out_channels=1, kernel_size=1, stride=1, bn=False, lrelu=False)
        self.convtf_var = conv_bn_relu(in_channels=128, out_channels=1, kernel_size=1, stride=1, bn=False, lrelu=False)
