...
with open("/workspace/evaluating_bdl/toyClassification/HMC/false_prob_values.pkl", "rb") as file: # (needed for python3)
...
for M in M_values:
    ...
    for l in range(L):
        network = ToyNet("eval_MC-Dropout-MAP-02-SGDMOM_1_M10", project_dir="/workspace/evaluating_bdl/toyClassification").cuda()
        network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyClassification/training_logs/model_MC-Dropout-MAP-02-SGDMOM_1_M10_%d/checkpoints/model_MC-Dropout-MAP-02-SGDMOM_1_M10_epoch_300.pth" % l))
