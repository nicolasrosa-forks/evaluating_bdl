...
networks = []
for i in range(M):
    network = ToyNet("eval_Ensemble-Adam-Fixed_1_M4", project_dir="/workspace/evaluating_bdl/toyClassification").cuda()
    network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyClassification/training_logs/model_Ensemble-Adam-Fixed_1_M4_%d/checkpoints/model_Ensemble-Adam-Fixed_1_M4_epoch_150.pth" % i))
