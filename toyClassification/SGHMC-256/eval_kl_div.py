...
for M in M_values:
    ...
    for j in range(10):
        ...
        for i in range(M):
            ...
            network = ToyNet("eval_SGHMC-256_1-10", project_dir="/workspace/evaluating_bdl/toyClassification").cuda()
            network.load_state_dict(torch.load("/workspace/evaluating_bdl/toyClassification/training_logs/model_SGHMC-256_%d/checkpoints/model_SGHMC-256_%d_epoch_%d.pth" % (j+1, j+1, int(num_epochs - i*step_size))))
